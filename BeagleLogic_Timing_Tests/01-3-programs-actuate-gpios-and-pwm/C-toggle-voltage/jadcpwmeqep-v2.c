...
float shitty_read_adc() {
  float value = 9999;
  FILE* fh = fopen("/sys/bus/iio/devices/iio:device0/in_voltage0_raw", "r");
  err = fscanf(fh, "%f", value);
  if (err == EOF) {
    printf("%s: %s (%d): Error reading adc!\n",__FILE__,__FUNCTION__,__LINE__);
  }
  fclose(fh);
  return value;
}
