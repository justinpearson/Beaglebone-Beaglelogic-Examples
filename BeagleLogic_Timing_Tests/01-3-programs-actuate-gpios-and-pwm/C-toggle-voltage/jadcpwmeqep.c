...
int get_spi_bus_path_number(unsigned int spi)
{
  ...
  build_path("/sys/devices", "ocp.3", ocp_dir, sizeof(ocp_dir));
...
int load_device_tree(const char *name)
{
    ...
    build_path("/sys/devices", "bone_capemgr.9", ctrl_dir, sizeof(ctrl_dir));
    snprintf(slots, sizeof(slots), "%s/slots", ctrl_dir);

    file = fopen(slots, "r+");
    if (!file) {
        // PyErr_SetFromErrnoWithFilename(PyExc_IOError, slots);
      printf("%s %s (%d): Can't open the slots file: %s\n",__FILE__,__func__,__LINE__, slots);
        return 0;
    }
